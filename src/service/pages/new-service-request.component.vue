...
const equipments = ref<Array<InstanceType<typeof Equipment>>>([])
...
onMounted(async () => {
  try {
    ...
    equipments.value = equipmentService.mapEquipment(response.data)
  } catch (error) {
    console.error('Error fetching equipment:', error)
    alert('Could not load the equipment. Please try again later.')
  }
})
...
async function handleSubmit() {
  try {
    const payload = {
      ...
      userId: null,
      companyId: null,
      equipmentId: formData.value.equipmentId || null,
      technicianId: null,
      serviceType: formData.value.serviceType || '',
      urgency: formData.value.urgency || 'normal',
      asap: formData.value.asap || false,
      timeSlot: formData.value.timeSlot || '',
      serviceAddress: formData.value.serviceAddress || '',
      scheduledDate: formData.value.date ? new Date(formData.value.date).toISOString() : null,
      completionDate: null,
      resolution: null
    }

    const response = await serviceRequestService.createRequest(payload)
